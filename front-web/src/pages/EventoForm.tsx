...
const EventoForm: React.FC = () => {
  ...
  const fetchEvento = async (eventoId: number) => {
    try {
      const evento = await eventoService.getById(eventoId);
      // Carregar imagens do localStorage
      const imagensSalvas = imageStorage.getEventoImages(eventoId);
      setFormData({
        nome: evento.nome,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditing && id) {
        await eventoService.update(parseInt(id), dadosSemImagens);
        // Salvar imagens no localStorage
        const salvou = imageStorage.saveEventoImages(parseInt(id), imagensComprimidas);
        if (!salvou) {
          toast.error('Erro ao salvar imagens. Espaço insuficiente.');
        }
        toast.success('Evento atualizado com sucesso!');
      } else {
        const novoEvento = await eventoService.create(dadosSemImagens);
        // Salvar imagens no localStorage com o ID do novo evento
        if (novoEvento && novoEvento.id) {
          const salvou = imageStorage.saveEventoImages(novoEvento.id, imagensComprimidas);
          if (!salvou) {
            toast.error('Evento criado, mas houve erro ao salvar imagens.');
          }
        }
  ...
  if (parques.length === 0) {
    return (
      <div className="max-w-2xl mx-auto text-center py-12">
        ...
        <p className="text-gray-500 mb-6">Você precisa cadastrar pelo menos um parque antes de criar um evento.</p>
        <button
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          <div>
            ...
            <input
              ...
              placeholder="Ex: Festival da Natureza"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Descreva o evento, programação, atividades..."
            />
          </div>
